...
export function isSupabaseConfigured() {
  const cleanUrl = cleanSupabaseUrl(process.env.NEXT_PUBLIC_SUPABASE_URL);
  const cleanKey = cleanSupabaseKey(process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY);
...
export async function getSupabaseServerClient() {
  ...
  const cleanUrl = cleanSupabaseUrl(rawUrl) || "https://placeholder-project.supabase.co";
...
export function getSupabaseAdminClient() {
  ...
  const cleanUrl = cleanSupabaseUrl(rawUrl) || "https://placeholder-project.supabase.co";
  const cleanKey = cleanSupabaseKey(serviceKey) || cleanSupabaseKey(process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) || "placeholder_service_role_key";
